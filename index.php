...
//if (KEY != '' && !isset($_GET['key']) && $_GET['key'] != KEY) header('location: /');
    
if(!function_exists('shell_exec')) die('Sorry, this server has blocked shell access :(');
...
if(!empty($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest' && isset($_POST['command'])) {
    ...
    if (!isset($_COOKIE["terminal_ollama"])) {
       print_r(json_encode((object) ['result' => 'NO AUTORIZADO', 'path' => $terminal->pwd()]));
       exit;
    }    
    ...
    <script type="text/javascript">
        ...
        $(document).keydown(function(e) {
            ...
            if (keyCode === 13){
                ...
                /* login */
                if(command.length !== 0 && login ==''){
                    login=command;
                    console.log("longitud->"+login.length);
                    command='';
                    endLine();
                    reset();
                    $('terminal content').append('<line class="current">PASSW: <sp></sp> <t><bl></bl></t></line>');
                    return;     
                ...
                /* password */
                if(command.length !== 0 && login !='' && password ==''){
                    password=command;
                    command='';
                    endLine();
                    if(!chekCredentials())return;
                ...
                /* envio de comando comunes al server */
                if(command.length !== 0 && login != '' && password != ''){
                    $.ajax({
                        type: 'POST',
                        async: false,
        ...
        ///////////////////////////////////////////////////////////////////////////////////////////////////////
        function chekCredentials(){
            
             
            /* se envia el login y password al backend para verificar */
            if(login != '' && password != ''){
                ...
                $.ajax({
                    ...
                    success: function( response ){
                        ...
                        if(response=="authorized"){ 
                            $('terminal content').append('<line class="current"><path><?php echo $terminal->pwd();?></path> <sp></sp> <t><bl></bl></t></line>');
                            return true;
                        }    
                        ...
                        if(response=="denied"){
                            login='';
                            password='';
                            $('terminal content').append('<line>##############################</line>');
                            $('terminal content').append('<line>#..CREDENCIALES INCORRECTAS..#</line>');
                            $('terminal content').append('<line>##############################</line>');
                            $('terminal content').append('<line class="current">LOGIN: <sp></sp> <t><bl></bl></t></line>');
                            return false;     
        ...
        function owner(){
            $('terminal content').append('<line>Marcelo Moglione</line>');
        }
        
        
        function logout(){
            document.cookie = "terminal_ollama=; expires=Thu, 01 Jan 1970 00:00:00 UTC;";
            location.reload();
  ...
  <body>
    <terminal>
      <header>
        ...
        <title>Terminal&nbsp; <?php echo '('.($terminal->whoami() ? $terminal->whoami() : '').($terminal->whoami() && $terminal->hostname() ? '@'.$terminal->hostname() : '').')';?></title>
      </header>
      <content>
        
      </content>
    </terminal>
    <footer>Coded by <a href="https://escuelitacoder.com">moglione@gmail.com</a></footer>
  </body>
  <script>
